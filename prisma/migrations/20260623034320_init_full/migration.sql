...
-- Materialized view for trending tags
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_trending_tags AS
WITH normalized AS (
    SELECT
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_trending_tag_norm
    ON mv_trending_tags (tag_norm);

CREATE INDEX IF NOT EXISTS idx_mv_trending_score
    ON mv_trending_tags (score DESC);

CREATE INDEX IF NOT EXISTS idx_mv_trending_count
    ON mv_trending_tags (count DESC);