...
export async function notifyAdmins(data: {
    ...
    const emailPromises = admins
        ...
        .map((admin) => {
            ...
            // Note: startSpan callback may not return the inner promise in all Sentry versions;
            // to be safe, call the email function directly here as well and return its promise.
            return sendNotificationEmail({
                to: admin.email,
                userName: admin.name || "Admin",
                notificationMessage: data.message,
