...
export async function performBasicMalwareCheck(
    buffer: Buffer,
    filename: string
): Promise<{ safe: boolean; error?: string }> {
  if (!buffer || buffer.length === 0) {
    return { safe: false, error: "Empty file" };
  }

  if (buffer.length > 100 * 1024 * 1024) {
    return { safe: false, error: "File too large for security check" };
  }

  const lower = filename.toLowerCase();
  const dangerousExtensions = [".exe", ".bat", ".cmd", ".sh", ".php", ".ps1", ".vbs", ".jar"];
  if (dangerousExtensions.some((ext) => lower.endsWith(ext))) {
    return { safe: false, error: "File extension not permitted" };
  }

  const sample = buffer.slice(0, 4096).toString("utf8", 0, Math.min(buffer.length, 4096));
  for (const pattern of DANGEROUS_PATTERNS) {
    if (pattern.test(sample)) {
      return { safe: false, error: "File contains potentially dangerous content" };
    }
  }
