    ...
    if (!mounted) {
        return (
            <div className="grid gap-4 sm:gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                {[0, 1, 2].map((i) => (
                    <div key={i} className="h-[280px] rounded-xl bg-gray-100 dark:bg-gray-800 animate-pulse" />
                ))}
            </div>
        );
    }
                ...
                <CardContent className="pb-4 sm:pb-5">
                    {userPieData.length > 0 ? (
                        <ResponsiveContainer width="100%" height={200}>
                            <PieChart>
                                <Pie data={userPieData} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={4} dataKey="value" label={({ name, value }) => `${name}: ${value}`}>
                        ...
                        <div className="h-[200px] flex items-center justify-center text-sm text-gray-400">No user data</div>
                    )}
                </CardContent>
            </Card>
            ...
            <Card className="border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 rounded-xl py-0 gap-0 overflow-hidden">
                ...
                <CardContent className="pb-4 sm:pb-5">
                    {projectPieData.length > 0 ? (
                        <ResponsiveContainer width="100%" height={200}>
                            <PieChart>
                                <Pie data={projectPieData} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={4} dataKey="value" label={({ name, value }) => `${name}: ${value}`}>
                        ...
                        <div className="h-[200px] flex items-center justify-center text-sm text-gray-400">No protocol data</div>
                    )}
                </CardContent>
            </Card>
