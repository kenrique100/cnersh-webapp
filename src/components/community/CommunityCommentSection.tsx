...
function TopicView({
    ...
    React.useEffect(() => {
        ...
        prevRepliesLengthRef.current = current;
        ...
        if (isNearBottom(el)) {
            ...
            setUnseenCount((c) => c + (current - prev));
                                ...
                                <div className="absolute bottom-full right-0 mb-2 bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg shadow-xl p-3 w-56 sm:w-64 z-50 max-w-[calc(100vw-2rem)]">
                                    <div className="grid grid-cols-8 gap-1">
                                        {EMOJI_LIST.map((emoji) => (
                                            <button key={emoji} onClick={() => handleEmoji(emoji)} className="h-8 w-8 flex items-center justify-center rounded hover:bg-gray-200 dark:hover:bg-gray-700 text-lg">
                                                {emoji}
                                            </button>
                                        ))}
                                    </div>
