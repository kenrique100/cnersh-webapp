...
export async function reactToReply(replyId: string, emoji: string) {
    const session = await authSession();
    if (!session) throw new Error("Unauthorized");
    ...
    const reactions = ((reply.reactions as Record<string, string[]>) || {});
    const emojiUsers = reactions[emoji] || [];

    if (emojiUsers.includes(session.user.id)) {
        // Toggle off — remove user from this emoji's list
        const updated = emojiUsers.filter((id) => id !== session.user.id);
        if (updated.length === 0) {
        ...
        reactions[emoji] = [...emojiUsers, session.user.id];
    }
