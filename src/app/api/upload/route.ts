...
async function uploadHandler(req: NextRequest): Promise<NextResponse> {
  ...
  if (file.type === "application/pdf") {
    try {
      ...
      if (pageCount === 0) {
        return NextResponse.json({ error: "PDF is empty (0 pages). Please upload a PDF with at least 1 page." }, { status: 400 });
      }
      if (pageCount > 4) {
        return NextResponse.json({ error: `PDF has ${pageCount} pages. Maximum allowed is 4 pages.` }, { status: 400 });
      }
    } catch {
  ...
  try {
    const uploadFile = new File([new Uint8Array(fileBuffer)], sanitizedFilename, { type: file.type });
  ...
  try {
    ...
    // Prevent an orphaned UploadThing object when the database write fails.
    if (storageKey) {
      await utapi.deleteFiles(storageKey).catch((cleanupError) => {
        console.error("[upload] Failed to clean up orphaned storage object:", cleanupError);
      });
    }
