...
const AAR_REQUIREMENTS = {
  email: "minsantedros@yahoo.com",
  reviewDays: 21,
} as const;
...
export default function AARClient({ projectId, projectTitle, application }: AARClientProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("AAR application submitted. Please also send by email to the DROS office and submit physical hard copies to MINSANTE.", { duration: 8000 });
      router.refresh();
    } catch (error) {
  ...
  return (
    <div className="w-full min-h-[calc(100vh-4rem)] bg-gray-50 dark:bg-gray-900">
      <div className="container mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 py-8">
        <Link href={`/protocols/${projectId}`} className="inline-flex items-center gap-1.5 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 mb-6 transition-colors">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">AAR Application</h1>
            <Badge className={statusInfo.color}>{statusInfo.label}</Badge>
          </div>
          <p className="text-sm text-gray-600 dark:text-gray-400">Protocol: <span className="font-medium">{projectTitle}</span></p>
          {application.aarRefNumber && <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">AAR Reference: <span className="font-mono font-bold">{application.aarRefNumber}</span></p>}
        </div>
            ...
            <div className="flex items-start gap-2 text-sm text-blue-700 dark:text-blue-400">
              ...
              <div className="space-y-1">
                ...
                <ol className="list-decimal list-inside space-y-0.5 ml-2 text-xs">
                  <li>Email your AAR documents to <strong>{AAR_REQUIREMENTS.email}</strong></li>
                  <li>Submit physical hard copies to MINSANTE in person</li>
                </ol>
                <p className="text-xs mt-1">⚠️ The {AAR_REQUIREMENTS.reviewDays} working-day DROS review clock starts when the DROS Officer confirms receipt — not when you submit this application.</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <Card className="border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950 rounded-xl mb-4">
            <CardContent className="pt-4">
              <div className="flex items-center gap-2 text-sm text-emerald-700 dark:text-emerald-400">
                ...
                <p>Application submitted on <strong>{application.submittedAt ? new Date(application.submittedAt).toLocaleDateString() : "—"}</strong>. Current status: <strong>{statusInfo.label}</strong>.</p>
              </div>
            </CardContent>
          </Card>
        )}
        ...
        {isDraft && (
          <Card className="border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 rounded-xl">
            <CardHeader>
              ...
              <CardDescription>Add any notes relevant to your AAR application. Ensure all required documents have been assembled before submitting.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Notes (optional)</label>
                <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Add any relevant notes about your AAR document set or submission status..." className="min-h-[100px]" />
              </div>
